...
namespace PakCreator
{
    public partial class FormMain : Form
    {
        ...
        private void btnStart_Click(object sender, EventArgs e)
        {
            ...
            StringCollection files_in_temp = new StringCollection();
            //
            ...
            {
                filecounter++;
                string tf = Program.ATemp.AppTempDir + StringUtils.PadNumToLength(filecounter, 10);
                string fn = fi.FullName;
                Log("Copy " + Path.GetFileName(fn));
                File.Copy(fi.FullName, tf);
                files_in_temp.Add(tf);
                //
                fn = fn.Replace(startpath, "");
                fn = fn.Replace("\\", "/"); // prevest normalni lomitka na unixovy
                file_index.Add(fn + "=" + Path.GetFileName(tf));
                Log("To: " + StringUtils.PadNumToLength(filecounter, 10) + " in Temp dir");
            }
            Log("Files pre-indexed");
            QuakePAK.CreateNewPak(pak_file);
            QuakePAK NewPAK = new QuakePAK(pak_file, true);
            Log("Created a new PAK: " + pak_file);
            foreach (string tempfile in files_in_temp)
            {
                string p = Path.GetFileName(tempfile);
                Log("Adding to PAK: " + p);
                NewPAK.AddFile(tempfile, p);
                Log("Deleting temp file: " + p);
                try
                {
